...
import com.R;
import com.adapter.FirstClassAdapter;
import com.adapter.SecondClassAdapter;
import com.adapter.StickyListAdapter;
import com.bean.FirstClassItem;
import com.bean.SecondClassItem;
...
public class StickyListActivity extends BaseActivity implements View.OnClickListener {

    @BindView(R.id.sl_list)
    StickyListHeadersListView mSlList;
    private List<TextTimeBean> mTextTimeBeens=new ArrayList<>();
    private PopupWindow popupWindow;
    //弹出PopupWindow时，背景变暗的动画
    private Animation animIn, animOut;
    private ArrayList<FirstClassItem> firstList;

    //弹出PopupWindow时背景变暗
    /*private View darkView;*/
    private ListView leftLV;
    private ListView rightLV;
    private ArrayList<SecondClassItem> secondList;

    @Override
    protected int getLayoutResId() {
        return R.layout.activity_sticky_list;
    }

    @Override
    protected void initView() {
        setBack();
        setTitle("测试StickyList");
        initData();
        StickyListAdapter adapter = new StickyListAdapter(this,mTextTimeBeens);
        mSlList.setAdapter(adapter);
        setRightBtn(R.mipmap.load_error);
    ...
    private void initData() {
        TextTimeBean mbean=null;
        for(int i=0;i<32;i++){
            if(i<=9){
                mbean=new TextTimeBean();
                mbean.mTime="2016年10月10号";
                mbean.mContent="这是一条很神秘的内容----》"+i;
            }else if(i>9&&i<=25){
                mbean=new TextTimeBean();
                mbean.mTime="2016年10月11号";
                mbean.mContent="这是一条很神秘的内容----》"+i;
            }else if (i>25){
                mbean=new TextTimeBean();
                mbean.mTime="2016年10月12号";
                mbean.mContent="这是一条很神秘的内容----》"+i;
            }
            mTextTimeBeens.add(mbean);
        }
    }
    ...
    private void tab1OnClick() {
        if (popupWindow.isShowing()) {
            ...
            popupWindow.showAsDropDown(findViewById(R.id.iv_img_right));
    ...
    private void setWindPlp(float v) {
        WindowManager.LayoutParams params=getWindow().getAttributes();
        params.alpha=v;
        getWindow().setAttributes(params);
    }

    private void initDataRes() {
    ...
    private void initPopup() {
        ...
        popupWindow.setHeight(getMobileHeight() * 2 / 3);
        popupWindow.setWidth(getMobileWidth());
